...
- A provider whose windows quota-axi cannot combine into one bound (Cursor, Copilot: `effectiveAvailability` is empty) has no effective percentage, pace, or runway to show, so its card replaces the headline block with a single `per-window usage · no combined bound` line and leads straight into its real per-window rows. No headroom, pace, or runway number is invented for it.
