...
Source attempts can include `credentialPresent` when a source is not genuinely absent, including when a read failure prevents a more precise classification. The [Provider `state` rules](#provider-state) define how attempts, including `degraded: false` recoveries, contribute to `state.degradedSources`.
...
| MiniMax                | Reports Token Plan `model_remains` windows, or a balance in `credits.remaining` with unit `usd`. Model-window IDs use `model:<slug>:5h` or `:7d` only for those exact reported durations, otherwise `:window:<duration>`; missing durations retain `:window:current_interval` or `:window:weekly` without invented cycle lengths. Explicit remaining percentages take precedence over counts; legacy `usage_count` is a remaining count. A window with status 3 and both counts zero is not allocated and is omitted even if it claims 100% remaining. A response consisting entirely of unallocated models is fresh with empty windows, clearing obsolete cache under the [Cache rules](#cache). See [Quota windows](#quota-windows) for scope relationships.                                                                                                                                                                                               |
...
| DeepSeek               | Reports `credits.remaining` from the USD total with unit `usd`, or, when no representable USD total exists, from the CNY total with unit `credits`. It does not combine or convert currencies, and reports no quota windows or resets. Effective semantics follow [Quota windows](#quota-windows).                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           |
...
- Missing or invalid credentials without a usable access token and usage HTTP 401 normally bypass and best-effort retire Claude cache. When macOS Keychain access remains unresolved, however, a 401 from a non-Keychain candidate such as an expired oauth-file sidecar rejects only that candidate. quota-axi preserves the skipped Keychain reason (permission withheld, denied, timed out, unreachable, or presence-check failure), does not classify Claude as signed out, and preserves any eligible stale cache. In particular, `security` exit 44 means `keychain_unreachable`, not absence. HTTP 403, timeout, network, rate-limit, server, and response-compatibility failures may use only a formerly fresh Claude snapshot less than seven days old that was captured for the same locally selected Claude configuration context (see [Cache](#cache)). Reset-expired windows are removed; resetless session, monthly, and credit windows expire after five hours, resetless weekly and model windows expire after seven days, and resetless unknown windows are rejected. A stale Claude attention row names `fetch failed` plus the usage-fetch error rather than a bare stale label.
...
- It sends one redirect-disabled `GET` per attempted credential to the fixed `https://api.kimi.com/coding/v1/usages` endpoint, sharing a 15 second total deadline across handover and enforcing a 262,144-byte decoded-body cap per response.
...
- It sends one redirect-disabled `GET` to that host's `/api/monitor/usage/quota/limit` with the key in a bare `Authorization` header (no `Bearer` prefix), a 15 second total deadline, and a 262,144-byte decoded-body cap. The endpoint is undocumented, so normalization is deliberately schema-tolerant rather than positional: `TOKENS_LIMIT` unit/number pairs identify token windows, `TIME_LIMIT` identifies the MCP-month window, and unrecognized limits are reported as untrusted unknown windows.
...
| Claude cache fallback                  | Follows the [Claude provider rules](#provider-notes), including unresolved Keychain access. Eligible fallback uses a formerly fresh snapshot from the same selected Claude configuration context, with a seven-day provider bound plus reset and resetless-window pruning. Its cache-only SHA-256 context identifier is opaque; legacy context-less Claude records are not reused. |
