...
auth[23]{provider,source,path,status,error}:
...
- It never launches the MiniMax CLI, performs a refresh-token exchange, writes credentials, or infers a quota from an inference request. Network failures may reuse a formerly fresh snapshot that was captured for the same credential source and deployment host (see [Cache](#cache)); definitive provider-wide auth failure retires that cache only after candidates are exhausted.
