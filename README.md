...
- Cursor Desktop is not required. On a CLI-only machine the editor `state.vscdb` is absent, so quota refreshes from the CLI Keychain token and the quota attempt is named `cli-keychain` in `sourcesTried`; the editor-credential fetch keeps its `api` attempt name. When no source can answer, the reported error comes from a source that still holds a credential in preference to a merely absent store, so a signed-in `cursor-agent` user sees the Keychain remedy rather than `Cursor sign-in required`.
