...
- At most one live delegated command for the same executable and argv in a quota-axi process. A timed-out child remains supervised until its terminal event, and repeated live-TUI reads do not stack another refresh on that unfinished run. A later read may delegate again only after the prior vendor command exits.
