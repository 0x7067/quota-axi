...
| Cursor                 | Can report `included_usage`, `auto_usage`, `api_usage`, and optional `spend_limit` windows, which share one plan billing cycle and are treated as jointly bounding, so effective `all_models` remaining is the lowest of them. Monthly labels alone are not trusted cycle evidence, so pace stays `unknown` unless a future provider duration appears.                                                                                                                                                                                                                 |
