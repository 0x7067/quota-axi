...
- CLI plumbing (routing, `--help`, `-v/--version`, error framing, exit codes, and the built-in `update` self-updater) comes from `axi-sdk-js` `runAxiCli`, matching sibling tasks-axi; machine TOON/JSON rendering stays in `src/render.ts` and the command bodies live in `src/commands.ts`.
...
- `--tui` (quota command only, mutually exclusive with `--json`) renders the human card-grid report from the pure renderer in `src/tui.ts`; it is presentation only, documented in [README Human terminal report](README.md#human-terminal-report---tui), and must never change the TOON/JSON machine contract, exit codes, or caching.
