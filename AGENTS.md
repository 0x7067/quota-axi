...
- Cursor has two independent, additive credential sources: the editor `state-vscdb` store and, on macOS, the Cursor CLI (`cursor-agent`) `cli-keychain` source in `src/providers/cursor-cli-credential.ts` (identity from `cli-config.json`, access token from the `cursor-access-token`/`cursor-user` Keychain item behind the same `--allow-keychain-prompt` gate and access-marker pattern Claude uses). Either source alone makes Cursor usable for both auth inspection and quota refresh (Cursor Desktop is never required), and the non-prompting editor source is tried first; a CLI-resolved quota attempt is named `cli-keychain` while the editor one keeps the historical `api` name. quota-axi never reads `cursor-refresh-token` and never refreshes Cursor credentials; CLI access-token refresh is an intentional, documented gap (expired token => `Cursor sign-in required`, remedy `cursor-agent login`), so do not add a guessed refresh endpoint. See [README Provider notes](README.md#provider-notes).
