...
- Cursor has two independent, additive credential sources: the editor `state-vscdb` store and, on macOS, the Cursor CLI (`cursor-agent`) `cli-keychain` source in `src/providers/cursor-cli-credential.ts` (identity from `cli-config.json`, access token from the `cursor-access-token`/`cursor-user` Keychain item behind the same `--allow-keychain-prompt` gate and access-marker pattern Claude uses). Either source alone makes Cursor usable, and the non-prompting editor source is tried first. quota-axi never reads `cursor-refresh-token` and never refreshes Cursor credentials. See [README Provider notes](README.md#provider-notes).
