...
- Cursor's recognized windows share one plan billing cycle and are treated as jointly bounding in `src/interpretation.ts`: effective `all_models` remaining is the minimum across them, via the same shared `availability()` min-across-bounds helper the other providers use. Unfamiliar Cursor windows stay in `unresolvedWindowIds` (semantics `partial`), never join the minimum, and never create a bound of their own. Copilot stays unbounded.
