...
- Claude access-token `expiresAt` metadata is advisory: when a token exists, the existing read-only usage request determines validity. Missing or invalid credentials without a usable token and usage HTTP 401/403 retire Claude cache; only transient failures may use a formerly fresh, age-bounded snapshot after reset and resetless-window pruning.
...
- JSON report shape and quota interpretation are documented in [README Output Model](README.md#output-model). Stale raw windows remain diagnostic but never expose known effective availability. `state.retryAfter` can appear for provider rate limits, and `state.reason: keychain_access_required` plus `state.remedyCommand` can appear when a stale or unavailable Claude result is blocked by a skipped macOS Keychain prompt.
