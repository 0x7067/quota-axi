...
- JSON provider reports include `provider`, `label`, `source`, `windows`, `quotaSemantics`, and `state`; consumers should use `quotaSemantics.effectiveAvailability` instead of isolated model-window percentages, and treat `partial` or `unknown` relationships as unresolved. `state.retryAfter` can appear for provider rate limits, and `state.reason: keychain_access_required` plus `state.remedyCommand` can appear when a stale or unavailable Claude result is blocked by a skipped macOS Keychain prompt.
